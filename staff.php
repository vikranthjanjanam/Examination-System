...
<html>
	...
	<?php
            if($_SERVER['REQUEST_METHOD'] == 'POST'){
                if(isset($_POST['Login'])){
                    ...
                    $result = $mysqli->query("SELECT * from staff WHERE StaffID = '$userID'");
            ...
            if(isset($_POST['Rst'])){
                ...
                $pres = $mysqli->query("SELECT * FROM `staff` WHERE `StaffID` = '$regId'");
                ...
                if($hash==$userCode['Code']){
                    $res = $mysqli->query("UPDATE `staff` SET `Password` = '$pass' WHERE `UserID` = '$regId'");
                    if ($res==TRUE){
	...
	<body>
		...
		<div class="top-header">
                    <div class="container">
			<a href="staff.php" id="headr" onclick="hideError();">
		...
		<div id="bottom-header">
			...
			<ul class="menu" onclick="hideError();">
                            <li><a href="#" class="actual" pid="OtherLogin" onclick="changeMenu(this);">Staff Login</a></li>
                            <div class="clear"/>
			</ul>
                    </nav>
		</div>
		...
		<div id="login">
                    <h1>Faculty Login</h1>
                    <form name="faculty" action="staff.php" method="post" autocomplete="off">
			<input name="userId" type="text" placeholder="User ID" required/>
			<input name="pass" type="password" placeholder="Password" onblur='validPassword("faculty")' required/>
                        <input type="submit" name="Login" value="Log in" onclick='validPassword("faculty")'/>
			...
			<div id="login">
				<h1>Account Help</h1>
                                <form name="adminHelp" action="staff.php" method="post">
                ...
                <div class="pages" id="PassCodeVerify">
                    <div id="login">
                        <h1>Change Password</h1>
                            <form name="verify" action="staff.php" method="post" autocomplete="off"/>
